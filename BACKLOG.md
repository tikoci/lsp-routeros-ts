...
- 📋 **Vendor (or depend on) `canonicalize.ts`** — pre-vendor audit at [`docs/canonicalize-audit.md`](docs/canonicalize-audit.md). Audit's H4 / H6 / H7 / H8 all shipped upstream across [tikoci/rosetta@9be870b](https://github.com/tikoci/rosetta/commit/9be870b), [@7c3e6fb](https://github.com/tikoci/rosetta/commit/7c3e6fb), and [@e05b508](https://github.com/tikoci/rosetta/commit/e05b508): pluggable `CanonicalizeOptions { isVerb?: (token, parentPath) => boolean }`, `extractMentions()`, `confidence` field, and the universal-verb set stays active under a resolver (so the LSP only needs to add device-observed menu-specific verbs). Remaining roadmap (H1 lenient / H2 var / H3 paren / H5 block-value) tracked at [tikoci/rosetta#5](https://github.com/tikoci/rosetta/issues/5) — preserves the same options shape so we can pull future improvements by diff. LSP-side wiring sketch in audit § "Path forward": back `isVerb` with a cache populated from `/console/inspect highlight` `cmd-name` tokens; universal fallback covers cold-start. *[rosetta-join P2]*

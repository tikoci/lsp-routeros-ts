...
export function updateSettings(newSettings: LspSettings) {
	...
	if (isDirty)
		log.info(
			`<settings> {update} now using ${replacedSettings.baseUrl} ${replacedSettings.username} ${replacedSettings.password ? '***' : 'null'} timeout ${replacedSettings.apiTimeout} allowClientProvidedCredentials ${replacedSettings.allowClientProvidedCredentials} checkCertificates ${replacedSettings.checkCertificates}`,
		)
	return isDirty
}

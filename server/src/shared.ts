...
import {
	InitializeParams, InitializeResult, TextDocuments, Connection, TextDocumentSyncKind, ServerCapabilities, DidChangeConfigurationNotification, WorkspaceFoldersChangeEvent, DidChangeConfigurationParams, CompletionParams, CompletionItem, SemanticTokens, SemanticTokensBuilder,
	CompletionItemKind,
	InsertTextFormat,
...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const defaultSettings: LspSettings = require('../../default-configuration.json').routeroslsp;
export let globalSettings: LspSettings = defaultSettings;

/* Server Capabilites (e.g. what the LSP can do) */
...
export function startLspServer(_connection: Connection) {
	...
	connection.onInitialize((params: InitializeParams): InitializeResult => {
		...
		connection.console.log(
			`connection.onInitialize() CLIENT capabilities: ${JSON.stringify(clientCapabilities, null, 2)}`
		);
	...
	connection.languages.diagnostics.on(handleDiagnostics(connection));


	// command completion
	connection.onCompletion(onCompletionHandler(connection));
	connection.onCompletionResolve(onConmpletionResolveHandler(connection));

	// semantic tokens
	connection.onRequest("textDocument/semanticTokens/full", generateSemanticTokens(connection));

	// hover on char
	connection.onHover(onHoverHandler(connection));
...
const onConmpletionResolveHandler = (connection: Connection) => (item: CompletionItem): CompletionItem => {
	connection.console.log(
...
const onCompletionHandler = (connection: Connection) => async (params: CompletionParams) => {
	connection.console.log(
	...
	interface RouterOSCompletionResult { show: string, completion: string }
	const roscompletion : RouterOSCompletionResult[] = await fetchInspect(
		"completion",
		text.slice(0, offset),
		settings
	);
	...
	if (roscompletion) {
		roscompletion.forEach((item : RouterOSCompletionResult, index: number) => {
			if (item.show === "true") {
				completions.push({
...
// note: documents.onX handle doing a refresh() on them

const generateSemanticTokens = (connection: Connection) => async (params : SemanticTokensParams): Promise<SemanticTokens | null> => {
	const document = documents.get(params.textDocument.uri);
	if (!document) {
		connection.console.warn(
...
const onHoverHandler = (connection: Connection) => async (params: TextDocumentPositionParams): Promise<Hover | null> => {
...
/* Axios HTTP client support */
//axios.defaults.baseURL = 'https://api.example.com';
axios.defaults.headers.post['Content-Type'] = 'application/json';
axios.defaults.auth = { username : "lsp", password : "changeme" };
...
async function fetchInspect(
	...
	try {
		...
		let errorText = "Unknown Error in fetchInspect()";
		if (error) {
...
const handleDiagnostics = (connection: Connection) => async (params: DocumentDiagnosticParams) => {
	connection.console.log(
